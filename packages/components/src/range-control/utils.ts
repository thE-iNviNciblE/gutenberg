 ...
 */
import type { useUnimpededRangedNumberEntryProps } from './types';
...
export function useUnimpededRangedNumberEntry( {
	...
	onChange,
	value,
}: useUnimpededRangedNumberEntryProps ) {
	const ref = useRef();
	const isDiverging = useRef( false );
	/** @type {import('../input-control/types').InputChangeCallback}*/
	const changeHandler = ( nextValue: string | undefined ) => {
		let next: number = parseFloat( `${ nextValue }` );
