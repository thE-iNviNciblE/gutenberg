...
export type RangeControlProps< P > = MarksProps & {
	/**
	 * If this property is added, a DashIcon component will be rendered after
	 * the slider with the icon equal to afterIcon.
	 */
	...
	allowReset?: boolean;
	/**
	 * If this property is added, a DashIcon component will be rendered before
	 * the slider with the icon equal to beforeIcon.
	 */
	...
	/**
	 * Passed as a prop to the `NumberControl` component and is only applicable
	 * if `withInputField` is true. If true, while the number input has focus,
	 * pressing `UP` or `DOWN` along with the `SHIFT` key will change the value
	 * by the `shiftStep` value.
	 *
	 * @default true
	 */
	isShiftStepEnabled?: boolean;
	/**
	 * If this property is added, a label will be generated using label property
	 * as the content.
	 */
	label?: string;
	/**
	...
	/**
	 * A function that receives the new value. The value will be less than `max`
	 * and more than `min` unless a reset (enabled by `allowReset`) has
	 * occurred. In which case the value will be either that of
	 * `resetFallbackValue` if it has been specified or otherwise `undefined`.
	 *
	 * @default () => void
	 */
	...
	/**
	 * Passed as a prop to the `NumberControl` component and is only applicable
	 * if `withInputField` and `isShiftStepEnabled` are both true and while the
	 * number input has focus. Acts as a multiplier of `step`.
	 *
	 * @default 10
	 */
	shiftStep?: number;
	 ...
	 * slider control in the bottom sheet on mobile. To use the stepper set the
	 * type value as `stepper`. Defaults to slider if no option is provided.
	 */
	type?: 'stepper';
	/**
	 * Determines if the `input` number field will render next to the RangeControl. This is overridden to `false` when
...
export type useUnimpededRangedNumberEntryProps = {
	max: number;
	min: number;
