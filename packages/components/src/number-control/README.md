...
The minimum number of digits to use when displaying `value`. If `pad` is greater
than the number of digits in `value`, then `value` will be left-padded with `0`
characters. For example, if `value` is `5` and `pad` is `2`, then `05` will be
shown.
